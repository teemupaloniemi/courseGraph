    ...
    courses.forEach(course => {
        if (index === 29 || course.department === departmentIndex) {
            ...
            if (!departmentNodes.some(node => node.id === course.id)) {
                ...
                const opacity = highlightedCourses.length === 0 || highlightedCourses.includes(course.id) ? 0.9 : 0.1; // Default opacity for all nodes if no text, otherwise reduce opacity for non-highlighted nodes
